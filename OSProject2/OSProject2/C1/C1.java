...
public class C1 {
    ...
    public C1(String S0address, int S0port) {
        try {
    ...
    private void startClient()// Server Function
    {
        ...
        C1Helper SH = new C1Helper(server, bufferedWriter, latch, lock);
        Thread thread = new Thread(SH);
        thread.start();
        // Server2
        server = connectToServer(aS2, pS2);
        SH = new C1Helper(server, bufferedWriter, latch, lock);
    ...
    public static void main(String args[]) {
        ...
        C1 server = new C1("10.176.69.51", 5050);// Change S0addess to IP of dx20
    }
}