...
public class C5 {
    ...
    public C5(String S0address, int S0port) {
        try {
    ...
    private void startClient()// Server Function
    {
        ...
        C5Helper SH = new C5Helper(server, bufferedWriter, latch, lock);
        Thread thread = new Thread(SH);
        thread.start();
        // Server2
        server = connectToServer(aS2, pS2);
        SH = new C5Helper(server, bufferedWriter, latch, lock);
    ...
    public static void main(String args[]) {
        ...
        C5 server = new C5("10.176.69.51", 5050);// Change S0addess to IP of dx20
    }
}