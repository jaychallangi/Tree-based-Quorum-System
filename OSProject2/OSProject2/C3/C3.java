...
public class C3 {
    ...
    public C3(String S0address, int S0port) {
        try {
    ...
    private void startClient()// Server Function
    {
        ...
        C3Helper SH = new C3Helper(server, bufferedWriter, latch, lock);
        Thread thread = new Thread(SH);
        thread.start();
        // Server2
        server = connectToServer(aS2, pS2);
        SH = new C3Helper(server, bufferedWriter, latch, lock);
    ...
    public static void main(String args[]) {
        ...
        C3 server = new C3("10.176.69.51", 5050);// Change S0addess to IP of dx20
    }
}