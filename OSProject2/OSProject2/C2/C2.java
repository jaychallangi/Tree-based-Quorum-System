...
public class C2 {
    ...
    public C2(String S0address, int S0port) {
        try {
    ...
    private void startClient()// Server Function
    {
        ...
        C2Helper SH = new C2Helper(server, bufferedWriter, latch, lock);
        Thread thread = new Thread(SH);
        thread.start();
        // Server2
        server = connectToServer(aS2, pS2);
        SH = new C2Helper(server, bufferedWriter, latch, lock);
    ...
    public static void main(String args[]) {
        ...
        C2 server = new C2("10.176.69.51", 5050);// Change S0addess to IP of dx20
    }
}