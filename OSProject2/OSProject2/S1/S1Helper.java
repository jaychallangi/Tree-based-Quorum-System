...
public class S1Helper implements Runnable, Comparable<S1Helper> {

    public static final String serverName = "SERVER1";
    public static final String Quorum = " SERVER4 SERVER5"; // needs to forward related messages only to SERVER2
    public static ArrayList<S1Helper> serverThreads = new ArrayList<S1Helper>();
    public static ArrayList<S1Helper> clientThreads = new ArrayList<S1Helper>();
    public static PriorityQueue<S1Helper> clientWaitList = new PriorityQueue<S1Helper>();
    ...
    public int compareTo(S1Helper other) {
        return this.requestTime.compareTo(other.requestTime);
    }

    public S1Helper(Socket connection, BufferedWriter xS0, CountDownLatch xlatch, Object xlock) {
        try {
            ...
            try {
                ...
                // Server Connection
                    ...
                    if (incoming.contains("REQUEST")) {
                            ...
                            if (locked) {
                                ...
                                synchronized (lock) {
                                    ...
                                    for (S1Helper x : serverThreads) {
                                        ...
                                        } else if (x.name.contains("SERVER3")
                        ...
                        if (!skip && (splitter[1].contains("SERVER2")
                                || splitter[1].contains("SERVER4")
                                || splitter[1].contains("SERVER5"))) {
                            String temp = splitter[1].replace(" SERVER3", "");
                    ...
                    if (incoming.contains("REPLY")) {
                        ...
                        if (incoming.contains(serverName)) {
                        ...
                        } else if (incoming.contains("SERVER2") || incoming.contains("SERVER4")
                                || incoming.contains("SERVER5")) {
                            for (S1Helper x : serverThreads) {
                                if (x.name.contains("SERVER2")) {
                                    outgoing = incoming;
                                    x.writeToConnection(outgoing);
                            ...
                            // Propogate reply to the left
                        } else if (incoming.contains("SERVER3") || incoming.contains("SERVER6")
                                || incoming.contains("SERVER7")) {
                            for (S1Helper x : serverThreads) {
                                if (x.name.contains("SERVER3")) {
                                    outgoing = incoming;
                                    x.writeToConnection(outgoing);
